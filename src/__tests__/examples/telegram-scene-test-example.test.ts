...
describe("ProjectScene - Пример использования нового фреймворка", () => {
    ...
    sequenceTester.testSequence("Создание проекта", [
      ...
      // Шаг 2: Нажатие на кнопку создания проекта
      {
        ...
        assertions: (tester) => {
          ...
          // Проверка результатов
          expect(context.scene.session.step).toBe(
  ...
  describe("Пример 3: Использование прямых тестов", () => {
    it("должен обрабатывать выбор проекта", async () => {
            ...
            reply_markup: {
              inline_keyboard: [
                [
                  {
                    text: "Конкуренты",
                    callback_data: `competitors_${project.id}`,
                  },
                ],
                ...
                [
                  {
                    ...
                    callback_data: `start_parsing_${project.id}`,
                  },
                ],
              ],
            },
      ...
      expect(mockContext.scene.session.step).toBe(
        ScraperSceneStep.PROJECT_MENU
      );
