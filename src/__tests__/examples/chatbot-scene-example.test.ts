...
// import { ScraperSceneSessionData } from "../../types"; // Unused
import {
  SceneTester,
  SequenceTester,
  createMockContext,
  createMockAdapter,
  expectReplyWithText,
  // expectAnswerCbQuery, // Unused
  // expectSceneEnter, // Unused
  // expectSceneLeave, // Unused
  // generateTextHandlerTests, // Unused
    ...
    sceneTester.createTestSuite([
      {
          ...
          (scene as any).onChatWithReel = async (ctx: any) => {
            ...
            if (isNaN(projectId) || !reelId) {
              if (ctx.callbackQuery)
                await ctx.answerCbQuery("Ошибка: неверные параметры.");
              ctx.scene.reenter();
              return;
            }
            ...
            const embedding = await (
              scene as any
            ).embeddingsService.getEmbeddingByReelId(reelId);
        ...
        test: async (scene, context, _adapter) => {
  ...
  describe("Using Test Generators", () => {
    ...
    beforeEach(() => {
      ...
      (chatbotScene as any).onText = async (ctx: any) => {
        ...
        if (!session.currentReelId) {
          await ctx.reply("Пожалуйста, сначала выберите Reel для чата.", {});
          return;
        }
    ...
    it("should generate response for text message", async () => {
      ...
      expect(context.sendChatAction).toHaveBeenCalledWith("typing");
      expect(
        (chatbotScene as any).chatbotService.generateResponse
