...
(skipTests ? describe.skip : describe)("PGVector SQL Setup Tests", () => {
  ...
  it("should be able to search similar transcripts", async () => {
    ...
    // Создаем вектор в формате PostgreSQL: [x1,x2,x3,...]
    const testEmbeddingArray = Array(1536).fill(0).map(() => Math.random());
    ...
    await adapter.executeQuery(
      ...
      ["https://www.instagram.com/reel/" + testReelId, 1, "test", testReelId, "Test Reel"]
    );
    ...
    await adapter.executeQuery(
      `INSERT INTO transcript_embeddings (reel_id, transcript, embedding)
       VALUES ($1, $2, $3::vector)`,
      ["https://www.instagram.com/reel/" + testReelId, "This is a test transcript for SQL testing.", testEmbedding]
    );

    // Выполняем поиск похожих расшифровок
    const result = await adapter.executeQuery(
      `SELECT * FROM search_similar_transcripts($1::vector, 0.5, 5)`,
      [testEmbedding]
    );
    ...
    const foundTestEmbedding = result.rows.find(row => row.reel_id === "https://www.instagram.com/reel/" + testReelId);
    ...
    await adapter.executeQuery(
      `DELETE FROM reels WHERE reel_url = $1`,
      ["https://www.instagram.com/reel/" + testReelId]
    );
  });
});
