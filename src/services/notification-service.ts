  ...
  async sendNewReelsNotification(
    ...
    try {
      ...
      if (!settings || !settings.new_reels_enabled) {
        logger.info(
          `[NotificationService] Уведомления о новых Reels отключены для пользователя ${userId}`
        );
      ...
      if (!project) {
        logger.error(
          `[NotificationService] Проект с ID ${projectId} не найден`
        );
        await this.storage.close();
        return;
      }
      ...
      if (!user) {
        logger.error(
          `[NotificationService] Пользователь с ID ${userId} не найден`
        );
        await this.storage.close();
        return;
      }
      ...
      logger.info(
        `[NotificationService] Отправлено уведомление о ${newReels.length} новых Reels пользователю ${userId}`
      );
    } catch (error) {
      logger.error(
        `[NotificationService] Ошибка при отправке уведомления о новых Reels:`,
        error
      );
    } finally {
      await this.storage.close();
  ...
  async sendTrendNotification(
    ...
    try {
      ...
      if (!settings || !settings.trends_enabled) {
        logger.info(
          `[NotificationService] Уведомления о трендах отключены для пользователя ${userId}`
        );
        await this.storage.close();
        return;
      }
      ...
      logger.info(
        `[NotificationService] Отправлено уведомление о тренде пользователю ${userId}`
      );
    } catch (error) {
      logger.error(
        `[NotificationService] Ошибка при отправке уведомления о тренде:`,
        error
      );
    } finally {
      await this.storage.close();
  ...
  async sendWeeklyReport(userId: number, projectId: number): Promise<void> {
    try {
      ...
      if (!settings || !settings.weekly_report_enabled) {
        logger.info(
          `[NotificationService] Еженедельные отчеты отключены для пользователя ${userId}`
        );
        await this.storage.close();
        return;
      }
      ...
      logger.info(
        `[NotificationService] Отправлен еженедельный отчет пользователю ${userId}`
      );
    } catch (error) {
      logger.error(
        `[NotificationService] Ошибка при отправке еженедельного отчета:`,
        error
      );
    } finally {
      await this.storage.close();
