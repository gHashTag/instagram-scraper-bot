...
export class SchedulerService {
  ...
  private scheduleNewReelsCheck(): void {
      ...
      try {
        ...
        for (const user of users) {
          ...
          for (const project of projects) {
            ...
            const newReels = await this.storage.getNewReels(
              project.id,
            ...
            if (newReels.length > 0) {
              await this.notificationService.sendNewReelsNotification(
                user.id,
                project.id,
                newReels
              );
        ...
        logger.error(
          "[SchedulerService] Ошибка при проверке новых Reels:",
          error
        );
      } finally {
        ...
        logger.info(
          `[SchedulerService] Следующая проверка новых Reels запланирована через 24 часа`
        );
      }
    };
    ...
    logger.info(
      `[SchedulerService] Первая проверка новых Reels запланирована через 1 минуту`
    );
  }
  ...
  private scheduleWeeklyReports(): void {
    ...
    const sendWeeklyReports = async () => {
      ...
      try {
        ...
        logger.error(
          "[SchedulerService] Ошибка при отправке еженедельных отчетов:",
          error
        );
      } finally {
        await this.storage.close();
        ...
        logger.info(
          `[SchedulerService] Следующая отправка еженедельных отчетов запланирована через 7 дней`
        );
      }
    };
    ...
    logger.info(
      `[SchedulerService] Первая отправка еженедельных отчетов запланирована через 2 минуты`
    );
  }
  ...
  private scheduleTrendsAnalysis(): void {
    ...
    const analyzeTrends = async () => {
      ...
      try {
        ...
        logger.info(
          `[SchedulerService] Следующий анализ трендов запланирован через 3 дня`
        );
      }
    };
    ...
    logger.info(
      `[SchedulerService] Первый анализ трендов запланирован через 3 минуты`
    );
  }
   ...
   */
  private async analyzeTrendsForProject(_projectId: number) {
