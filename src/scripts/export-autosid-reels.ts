/**
 * Скрипт для экспорта Reels по хэштегам эстетической медицины в Excel для проекта АвтоСид
 *
 * Использование:
 * bun run src/scripts/export-autosid-reels.ts [minViews] [outputPath]
 *
 * Параметры:
 * - minViews: (опционально) Минимальное количество просмотров (по умолчанию 50000)
 * - outputPath: (опционально) Путь для сохранения Excel файла (по умолчанию exports/autosid_aesthetic_reels_<timestamp>.xlsx)
 */
...
const defaultFilename = `autosid_aesthetic_reels_${timestamp}.xlsx`;
const outputPath =
  args[1] || path.join(process.cwd(), "exports", defaultFilename);

if (isNaN(minViews)) {
  logger.error("Ошибка: minViews должно быть числом");
  process.exit(1);
}
...
async function main() {
  logger.info(
    `Экспорт Reels по хэштегам эстетической медицины для проекта ${projectId} (АвтоСид) с минимум ${minViews} просмотров`
  );
  ...
  try {
    ...
    // Получаем все Reels для проекта
    ...
    const unknownReels: any[] = [];

    for (const reel of reels) {
      // Проверяем, является ли source_identifier ID хэштега
      const hashtagId = reel.source_identifier;
      if (!hashtagId) {
        unknownReels.push(reel);
        continue;
      }
      if (hashtagMap.has(hashtagId)) {
        ...
        // Если source_identifier не соответствует ID хэштега, добавляем в группу "Другие"
        unknownReels.push(reel);
      }
    }

    // Если есть Reels без соответствующего хэштега, добавляем их в отдельную группу
    if (unknownReels.length > 0) {
      reelsByHashtag.set("unknown", unknownReels);
      hashtagMap.set("unknown", "Другие");
    }
