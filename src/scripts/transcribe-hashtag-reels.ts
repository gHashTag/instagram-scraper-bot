/**
 * Скрипт для транскрибации Reels по хэштегам
 *
 * Использование:
 * bun run src/scripts/transcribe-hashtag-reels.ts <projectId> [minViews] [daysBack] [limit]
 *
 * Параметры:
 ...
 * - limit: (опционально) Максимальное количество Reels для обработки (по умолчанию 5)
 */
...
if (args.length < 1) {
  console.error(
    "Использование: bun run src/scripts/transcribe-hashtag-reels.ts <projectId> [minViews] [daysBack] [limit]"
  );
  process.exit(1);
}
...
async function main() {
  console.log(
    `Транскрибация Reels по хэштегам для проекта ${projectId} с минимум ${minViews} просмотров за последние ${daysBack} дней`
  );
  ...
  try {
    ...
    try {
      // Получаем список Reels по хэштегам без транскрипций
      ...
      console.log(
        `Найдено ${reelsResult.rows.length} Reels по хэштегам без транскрипций`
      );

      if (reelsResult.rows.length === 0) {
        console.log("Нет Reels по хэштегам для транскрибации");
        return;
      }
      ...
      for (let i = 0; i < reelsResult.rows.length; i++) {
        ...
        console.log(
          `\n======= Обработка Reel ${i + 1}/${reelsResult.rows.length} =======`
        );
        console.log(
    ...
    console.error("Ошибка при транскрибации Reels по хэштегам:", error);
  } finally {
