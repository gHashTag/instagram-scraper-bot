 ...
 *
 * Использование:
 * bun run src/scripts/export-reels-by-hashtags.ts <projectId> [minViews] [outputPath]
 *
 * Параметры:
 * - projectId: ID проекта
 * - minViews: (опционально) Минимальное количество просмотров (по умолчанию 50000)
 * - outputPath: (опционально) Путь для сохранения Excel файла (по умолчанию exports/reels_by_hashtags_<timestamp>.xlsx)
 */

import { initializeDBConnection } from "../db/neonDB";
import { reelsTable, hashtagsTable, projectsTable } from "../db/schema";
...
if (args.length < 1) {
  logger.error(
    "Использование: bun run src/scripts/export-reels-by-hashtags.ts <projectId> [minViews] [outputPath]"
  );
  process.exit(1);
}
...
const defaultFilename = `reels_by_hashtags_${projectId}_${timestamp}.xlsx`;
const outputPath =
...
async function main() {
  try {
    ...
    logger.info(
      `Экспорт Reels по хэштегам для проекта ${projectId} (${projectName}) с минимум ${minViews} просмотров`
    );
    ...
    // Создаем карту хэштегов для быстрого доступа
    ...
    // Получаем все Reels для проекта с минимальным количеством просмотров
