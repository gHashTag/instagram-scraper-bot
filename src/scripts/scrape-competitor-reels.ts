/**
 * Скрипт для парсинга Reels конкурентов с использованием Apify API
 * 
 * Использование:
...
if (args.length < 2) {
  logger.error("Использование: bun run src/scripts/scrape-competitor-reels.ts <projectId> <competitorId> [limit]");
  process.exit(1);
}
