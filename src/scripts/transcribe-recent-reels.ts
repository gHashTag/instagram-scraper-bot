/**
 * Скрипт для транскрибации Reels за последний месяц с более чем 50,000 просмотров
 *
 * Использование:
 * bun run src/scripts/transcribe-recent-reels.ts <projectId> [minViews] [daysBack] [limit]
 *
 * Параметры:
...
if (args.length < 1) {
  console.error(
    "Использование: bun run src/scripts/transcribe-recent-reels.ts <projectId> [minViews] [daysBack] [limit]"
  );
  process.exit(1);
}
...
async function main() {
  console.log(
    `Транскрибация Reels за последний месяц с более чем ${minViews} просмотров`
  );
  ...
  try {
    ...
    // Получаем список Reels без транскрипций
    const result = await adapter.executeQuery(
      `SELECT id, reel_url, author_username, views_count FROM reels 
       WHERE project_id = $1 
    ...
    console.log(`Найдено ${result.rows.length} Reels без транскрипций`);

    if (result.rows.length === 0) {
      console.log("Нет Reels для транскрибации");
      return;
    }

    // Обрабатываем каждый Reel
    for (let i = 0; i < result.rows.length; i++) {
      const reel = result.rows[i];
      console.log(
        `\n======= Обработка Reel ${i + 1}/${result.rows.length} =======`
      );
