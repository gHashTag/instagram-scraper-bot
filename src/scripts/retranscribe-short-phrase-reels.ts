/**
 * Скрипт для повторной транскрибации рилсов с короткими фразами
 *
 * Использование:
 * bun run src/scripts/retranscribe-short-phrase-reels.ts <projectId> [minViews] [limit]
 *
 * Параметры:
...
if (args.length < 1) {
  console.error(
    "Использование: bun run src/scripts/retranscribe-short-phrase-reels.ts <projectId> [minViews] [limit]"
  );
  process.exit(1);
}
...
async function main() {
  console.log(
    `Повторная транскрибация рилсов с короткими фразами для проекта ${projectId} с минимум ${minViews} просмотров`
  );
  ...
  try {
    // Получаем список рилсов с короткими фразами
    const reelsResult = await adapter.executeQuery(
      `SELECT r.*, h.tag_name as hashtag_name 
       ...
       AND (
         r.transcript LIKE '%Спасибо за просмотр%' OR 
         r.transcript LIKE '%Благодарю за просмотр%' OR 
         r.transcript LIKE '%Редактор субтитров%' OR
         r.transcript IS NULL
       )
    ...
    console.log(
      `Найдено ${reelsResult.rows.length} рилсов с короткими фразами`
    );

    if (reelsResult.rows.length === 0) {
      console.log("Нет рилсов для повторной транскрибации");
      return;
    }
    ...
    for (let i = 0; i < reelsResult.rows.length; i++) {
      ...
      console.log(`Текущая транскрипция: ${reel.transcript || "Нет"}`);
    ...
    console.log("\nПовторная транскрибация завершена");
  } catch (error) {
    console.error("Ошибка при повторной транскрибации рилсов:", error);
  } finally {
