/**
 * Скрипт для создания финального отчета с транскрипциями Reels конкурентов
 *
 * Использование:
 * bun run src/scripts/generate-final-report.ts <project_id> <min_views> <days_ago> <output_path>
 *
 * Пример:
 * bun run src/scripts/generate-final-report.ts 1 50000 30 "exports/final_report.xlsx"
 */
...
const outputPath =
  process.argv[5] ||
  `exports/final_report_${format(new Date(), "yyyy-MM-dd")}.xlsx`;

// Интерфейс для объекта Reel из базы данных, должен соответствовать ReelContent
interface ReelFromDB extends ReelContent {}
...
async function generateReport() {
  console.log(
    `Создание финального отчета для проекта ${projectId} с минимальным количеством просмотров ${minViews} за последние ${daysAgo} дней`
  );
  ...
  try {
    ...
    const reels: ReelFromDB[] = result.rows as ReelFromDB[];
    ...
    const reelsWithRealTranscriptions = reels.filter(
      (reel) =>
        reel.transcript !== undefined && isRealTranscription(reel.transcript)
    );
    console.log(
    ...
    // Добавляем аналитику по конкурентам
    summarySheet.addRow([""]); // Пустая строка для разделения
    summarySheet.addRow(["📈 Аналитика по конкурентам", ""]);

    // Сортируем конкурентов по количеству Reels
    const competitorsWithReelsCount = competitors
      .map((competitor) => ({
        competitor,
        reelsCount: (reelsByCompetitor[competitor.id] || []).length,
        totalViews: (reelsByCompetitor[competitor.id] || []).reduce(
          (sum, reel) => sum + (reel.views ?? 0),
          0
        ),
        avgViews:
          (reelsByCompetitor[competitor.id] || []).length > 0
            ? Math.round(
                (reelsByCompetitor[competitor.id] || []).reduce(
                  (sum, reel) => sum + (reel.views ?? 0),
                  0
                ) / ((reelsByCompetitor[competitor.id] || []).length || 1)
              )
            : 0,
      }))
      .sort((a, b) => b.reelsCount - a.reelsCount);
    ...
    for (const {
      ...
      totalViews,
      avgViews,
    } of competitorsWithReelsCount) {
      summarySheet.addRow([
        `👤 ${competitor.username}`,
        `${reelsCount} Reels, ${formatNumber(totalViews)} просмотров, в среднем ${formatNumber(avgViews)} просмотров`,
      ]);
    }
    ...
    console.log(`Финальный отчет успешно сохранен в файл: ${outputPath}`);
  } catch (error) {
    console.error("Ошибка при создании финального отчета:", error);
  } finally {
