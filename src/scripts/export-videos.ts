/**
 * Скрипт для экспорта видео Reels в локальную директорию
 *
 * Использование:
 * bun run src/scripts/export-videos.ts <projectId> [sourceType] [sourceId] [limit] [outputDir]
 *
 * Параметры:
 ...
 * - outputDir: (опционально) Директория для сохранения видео (по умолчанию ./exports/videos)
 */
...
if (args.length < 1) {
  logger.error(
    "Использование: bun run src/scripts/export-videos.ts <projectId> [sourceType] [sourceId] [limit] [outputDir]"
  );
  process.exit(1);
}
...
const outputDir = args[4] || path.join(process.cwd(), "exports", "videos");
...
async function getReels(
  ...
  // Используем db.executeQuery вместо db.select()
  const query = `
    SELECT *
    FROM reels
    WHERE project_id = $1
    ...
    LIMIT $4
  `;
  const params = [projectId, sourceType, sourceId.toString(), limit]; // sourceId должен быть строкой для source_identifier
  try {
    const result = await db.executeQuery(query, params);
    return result.rows;
  } catch (error: any) {
    logger.error(`Ошибка при получении Reels: ${error.message}`);
    return [];
  }
}
 ...
 */
async function main() {
  logger.info(`Запуск экспорта видео для проекта ${projectId}`);
  ...
  try {
    ...
    const csvPath = path.join(
      outputDir,
      `reels_info_${projectId}_${sourceType}_${sourceId}_${new Date().toISOString().replace(/:/g, "-")}.csv`
    );
    const csvHeader =
    ...
    for (const reel of reels) {
      ...
      // Создаем имя файла на основе ID, автора и даты публикации
      const filename = `${reelId}_${authorUsername}_${publishedDate}.mp4`;
      const outputPath = path.join(outputDir, filename);

      logger.info(`Экспорт Reel: ${reelUrl} (ID: ${reelId}) -> ${filename}`);

      try {
        // Сначала пробуем скачать видео по URL Reel
        logger.info(`Скачивание видео с URL: ${reelUrl}`);
        try {
          await downloadReelVideo(reelUrl, outputPath);
          ...
          if (downloadError instanceof Error) {
            ...
            logger.warn(
              `Не удалось скачать видео с URL Reel: ${String(downloadError)}`
            );
          }
          ...
          if (reel.video_download_url) {
            ...
            await downloadVideoWithCurl(reel.video_download_url, outputPath);
          } else {
        ...
        const csvLine = `${reelId},"${filename}","${reelUrl}","${authorUsername}","${reel.published_at || ""}",${reel.views_count || 0},${reel.likes_count || 0},${reel.comments_count || 0},"${(reel.description || "").replace(/"/g, '""')}"\n`;
        fs.appendFileSync(csvPath, csvLine);

        successCount++;
        logger.info(
          `Успешно экспортирован Reel ${reelId} (${successCount}/${reels.length})`
        );
      } catch (error) {
        logger.error(`Ошибка при экспорте Reel ${reelId}:`, error);
        // Продолжаем с следующим Reel
      }
    }

    logger.info(
      `Экспорт видео завершен. Успешно экспортировано ${successCount} из ${reels.length} Reels.`
    );
    logger.info(`Информация о Reels сохранена в файле: ${csvPath}`);
  } catch (error) {
    logger.error("Критическая ошибка при выполнении экспорта видео:", error);
    process.exit(1);
  }
}
