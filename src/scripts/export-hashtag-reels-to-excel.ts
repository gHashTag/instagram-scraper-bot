/**
 * Скрипт для экспорта Reels по хэштегам в Excel файл
 *
 * Использование:
 * bun run src/scripts/export-hashtag-reels-to-excel.ts <projectId> [minViews] [outputPath]
 *
 * Параметры:
 * - projectId: ID проекта
 * - minViews: (опционально) Минимальное количество просмотров (по умолчанию 50000)
 * - outputPath: (опционально) Путь для сохранения Excel файла (по умолчанию exports/hashtag_reels_<projectId>_<timestamp>.xlsx)
 */
...
if (args.length < 1) {
  logger.error(
    "Использование: bun run src/scripts/export-hashtag-reels-to-excel.ts <projectId> [minViews] [outputPath]"
  );
  process.exit(1);
}
...
const defaultFilename = `hashtag_reels_${projectId}_${timestamp}.xlsx`;
const outputPath =
...
async function main() {
  logger.info(
    `Экспорт Reels по хэштегам для проекта ${projectId} с минимум ${minViews} просмотров`
  );
  ...
  try {
    ...
    for (const hashtag of hashtags) {
      const hashtagName = hashtag.tag_name;

      // Получаем Reels для текущего хэштега
      const reels = await db
        ...
        .where(
          and(
            ...
            eq(reelsTable.source_identifier, String(hashtag.id)),
            gte(reelsTable.views_count || 0, minViews)
          )
        );

      logger.info(
        `Найдено ${reels.length} Reels для хэштега #${hashtagName} с минимум ${minViews} просмотров`
      );

      // Если есть Reels, создаем лист для хэштега и добавляем статистику
      if (reels.length > 0) {
        ...
        const viewsArray = reels.map((reel) => reel.views_count || 0);
        const likesArray = reels.map((reel) => reel.likes_count || 0);
        const commentsArray = reels.map((reel) => reel.comments_count || 0);
