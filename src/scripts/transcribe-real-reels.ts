/**
 * Скрипт для транскрибации реальных рилсов из Instagram
 *
 * Использование:
 * bun run src/scripts/transcribe-real-reels.ts <projectId> <reelUrl>
 *
 * Параметры:
 * - projectId: ID проекта
 * - reelUrl: URL рила в Instagram
 */
...
if (args.length < 2) {
  console.error(
    "Использование: bun run src/scripts/transcribe-real-reels.ts <projectId> <reelUrl>"
  );
  process.exit(1);
}

const projectId = parseInt(args[0], 10);
const reelUrl = args[1];
...
// Основная функция
async function main() {
  console.log(`Транскрибация рила: ${reelUrl}`);
  ...
  try {
    // Скачиваем видео
    const videoPath = await downloadVideo(reelUrl);
    ...
    console.log("------------------------");
    console.log(improvedTranscription);
    console.log("------------------------");
    ...
    console.error("Ошибка при транскрибации рила:", error);
  } finally {
