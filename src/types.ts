...
export interface StorageAdapter {
  ...
  getProjectsByUserId(userId: string | number): Promise<Project[]>;
  createProject(userId: string | number, name: string): Promise<Project>;
  ...
  getReelsByCompetitorId(competitorId: number, filter: any): Promise<any[]>;
...
export interface InstagramScraperBotConfig {
  ...
  apifyToken?: string; // Добавлено для совместимости с src/bot.ts
  openaiApiKey?: string; // Добавлено для совместимости с src/bot.ts
  adminUserId?: number; // Добавлено для совместимости с src/bot.ts
...
export enum ScraperSceneStep {
  ...
  // Шаги для сцены транскрибации URL
  TRANSCRIBE_ENTER_URL = "TRANSCRIBE_ENTER_URL", // Шаг ввода URL для транскрибации
  TRANSCRIBE_PROCESSING = "TRANSCRIBE_PROCESSING", // Шаг обработки видео
  TRANSCRIBE_RESULT = "TRANSCRIBE_RESULT", // Шаг отображения результата транскрибации
...
export interface ScraperSceneSessionData extends Scenes.SceneSessionData {
  ...
  // Добавляем cursor для совместимости с WizardSessionData (обязательное поле)
  cursor: number;
  ...
  // Поля для сцены транскрибации URL
