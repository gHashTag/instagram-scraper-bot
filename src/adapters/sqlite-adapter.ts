...
export class SqliteAdapter implements StorageAdapter {
  ...
  async getProjectsByUserId(userId: string | number): Promise<Project[]> {
    ...
    try {
      // Для SQLite всегда используем number (integer ID)
      const numericUserId =
      ...
      const projects = query.all(numericUserId) as Project[];
      return projects;
    } catch (error) {
   ...
   */
  async createProject(userId: string | number, name: string): Promise<Project> {
    const db = this.ensureConnection();
    ...
    try {
      ...
      const result = query.run(numericUserId, name, null);
  ...
  private ensureMarketingColumnsExist(): void {
    ...
    try {
      ...
      const tableInfo = db
        .prepare("PRAGMA table_info(ReelsContent)")
        .all() as any[];
      ...
      for (const column of marketingColumns) {
        if (!columnNames.includes(column.name)) {
          db.prepare(
            `ALTER TABLE ReelsContent ADD COLUMN ${column.name} ${column.type}`
          ).run();
      ...
      console.error(
        "Ошибка при проверке/добавлении столбцов для маркетинговых данных:",
        error
      );
    }
  }
   ...
   * @param filter Параметры фильтрации
   */
  async getReelsWithMarketingData(
    filter: ReelsFilter = {}
  ): Promise<ReelContent[]> {
   ...
   */
  async calculateMarketingData(
    reel: ReelContent,
    averageFollowers: number = 10000
  ): Promise<ReelContent> {
    // Создаем экземпляр сервиса для расчета маркетинговых данных
    const marketingService = new MarketingAnalyticsService();

    // Рассчитываем маркетинговые данные
    const reelWithMarketingData = marketingService.calculateMarketingData(
      reel,
      averageFollowers
    );
  ...
  async updateReelMarketingData(
    ...
    try {
      ...
      const getReelStatement = db.prepare(
        "SELECT * FROM ReelsContent WHERE id = ?"
      );
      ...
      console.error(
        "Ошибка при обновлении маркетинговых данных Reel в SQLite:",
        error
      );
      return null;
    }
  ...
  async createReelsCollection(
    ...
    try {
      ...
      const getCollectionStatement = db.prepare(
        "SELECT * FROM ReelsCollections WHERE id = ?"
      );
      ...
      return {
        ...
        reels_ids: collection.reels_ids
          ? JSON.parse(collection.reels_ids)
          : undefined,
   ...
   */
  async getReelsCollectionsByProjectId(
    projectId: number
  ): Promise<ReelsCollection[]> {
    const db = this.ensureConnection();
    ...
    try {
      ...
      const statement = db.prepare(
        "SELECT * FROM ReelsCollections WHERE project_id = ? ORDER BY created_at DESC"
      );
  ...
  async getReelsByCompetitorId(
    ...
    try {
      ...
      console.error(
        "Ошибка при получении Reels по ID конкурента из SQLite:",
        error
      );
      return [];
    }
  ...
  async logParsingRun(log: Partial<ParsingRunLog>): Promise<ParsingRunLog> {
    ...
    try {
      ...
      console.error(
        "Ошибка при создании лога запуска парсинга в SQLite:",
        error
      );
      throw new Error(
  ...
  async updateParsingLog(log: Partial<ParsingRunLog>): Promise<ParsingRunLog> {
    ...
    try {
      ...
      console.error(
        "Ошибка при обновлении лога запуска парсинга в SQLite:",
        error
      );
      throw new Error(
  ...
  async getParsingRunLogs(
    ...
    try {
      ...
      console.error(
        "Ошибка при получении логов запуска парсинга из SQLite:",
        error
      );
      return [];
    }
  ...
  async getParsingLogsByProjectId(projectId: number): Promise<ParsingRunLog[]> {
    ...
    try {
      ...
      console.error(
        "Ошибка при получении логов запуска парсинга по ID проекта из SQLite:",
        error
      );
      return [];
    }
  ...
  async saveUser(userData: {
    ...
    console.log(
      `[DEBUG] SqliteAdapter.saveUser: Сохранение пользователя с telegramId=${userData.telegramId}`
    );
    try {
      ...
      if (existingUser) {
        console.log(
          `[DEBUG] SqliteAdapter.saveUser: Пользователь с telegramId=${userData.telegramId} уже существует, обновляем данные`
        );
        ...
        console.log(
          `[DEBUG] SqliteAdapter.saveUser: Пользователь с telegramId=${userData.telegramId} не найден, создаем нового`
        );
      ...
      console.error(
        "[ERROR] Ошибка при сохранении пользователя в SQLite:",
        error
      );
      throw new Error(
  ...
  async getNotificationSettings(userId: number): Promise<any | null> {
    console.warn(
      `[SqliteAdapter] getNotificationSettings не реализован для SQLite. userId: ${userId}`
    );
    return null;
  }
  ...
  async saveNotificationSettings(settings: any): Promise<any> {
    console.warn(
      `[SqliteAdapter] saveNotificationSettings не реализован для SQLite. settings: ${JSON.stringify(settings)}`
    );
    return settings;
  }
  ...
  async updateNotificationSettings(
    ...
    console.warn(
      `[SqliteAdapter] updateNotificationSettings не реализован для SQLite. userId: ${userId}, settings: ${JSON.stringify(settings)}`
    );
    return settings;
  }
  ...
  async getUserById(userId: number): Promise<User | null> {
    console.warn(
      `[SqliteAdapter] getUserById не реализован для SQLite. userId: ${userId}`
    );
    return null;
  }
  ...
  async getNewReels(
    ...
    console.warn(
      `[SqliteAdapter] getNewReels не реализован для SQLite. projectId: ${projectId}, afterDate: ${afterDate}`
    );
    return [];
  }
}
