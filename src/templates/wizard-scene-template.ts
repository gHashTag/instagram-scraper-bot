...
function clearSessionState(
  ...
  if (ctx.scene.session) {
    logger.info(
      `[TemplateWizard] Clearing session state before leaving (reason: ${reason})`
    );
...
async function safeSceneTransition(
  ...
  try {
    logger.info(
      `[TemplateWizard] Transitioning to ${targetScene} scene (reason: ${reason})`
    );
    await ctx.scene.enter(targetScene);
  } catch (error) {
    logger.error(
      `[TemplateWizard] Error entering ${targetScene} scene:`,
      error
    );
  ...
  constructor(_storage: StorageAdapter) {
    super(
      ...
      async (ctx) => {
        ...
        logger.debug(
          `[TemplateWizard] Содержимое ctx.wizard.state:`,
          ctx.wizard.state
        );
