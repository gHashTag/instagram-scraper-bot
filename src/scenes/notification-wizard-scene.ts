...
function clearSessionState(
  ...
  if (ctx.scene.session) {
    logger.info(
      `[NotificationWizard] Clearing session state before leaving (reason: ${reason})`
    );
 ...
 */
export const notificationWizardScene =
  new Scenes.WizardScene<ScraperBotContext>(
    "notification_wizard",

    // Шаг 1: Отображение настроек уведомлений
    async (ctx) => {
      logger.info(
        `[NotificationWizard] Шаг 1: Отображение настроек уведомлений`
      );
      logger.debug(
        `[NotificationWizard] Содержимое ctx.wizard.state:`,
        ctx.wizard.state
      );
      ...
      try {
        ...
        if (!user) {
          logger.error(
            `[NotificationWizard] User not found for telegram_id: ${ctx.from.id}`
          );
          await ctx.reply(
        ...
        if (!settings) {
          logger.info(
            `[NotificationWizard] Creating default notification settings for user ${user.id}`
          );
    ...
    async (ctx) => {
      ...
      if (ctx.message && "text" in ctx.message) {
        ...
        if (!userId) {
          ...
          await ctx.reply(
            "Ошибка: не удалось определить пользователя. Начните сначала."
          );
        ...
        try {
          ...
          logger.error(
            "[NotificationWizard] Error updating views threshold:",
            error
          );
          await ctx.reply(
    ...
    async (ctx) => {
      ...
      if (ctx.message && "text" in ctx.message) {
        ...
        try {
          ...
          await ctx.reply(
            `Время отправки уведомлений успешно изменено на ${time}.`
          );
          ...
          logger.error(
            "[NotificationWizard] Error updating notification time:",
            error
          );
          await ctx.reply(
...
notificationWizardScene.action(/toggle_new_reels_(on|off)/, async (ctx) => {
  logger.info(
    `[NotificationWizard] Обработчик кнопки 'toggle_new_reels' вызван`
  );
  await ctx.answerCbQuery();
  ...
  if (!userId) {
    logger.error(
      "[NotificationWizard] User ID is undefined in toggle_new_reels handler"
    );
    await ctx.reply(
      "Ошибка: не удалось определить пользователя. Начните сначала."
    );
  ...
  try {
    ...
    logger.error(
      "[NotificationWizard] Error in toggle_new_reels handler:",
      error
    );
    await ctx.reply(
...
notificationWizardScene.action(/toggle_trends_(on|off)/, async (ctx) => {
  ...
  if (!userId) {
    logger.error(
      "[NotificationWizard] User ID is undefined in toggle_trends handler"
    );
    await ctx.reply(
      "Ошибка: не удалось определить пользователя. Начните сначала."
    );
...
notificationWizardScene.action(/toggle_weekly_report_(on|off)/, async (ctx) => {
  logger.info(
    `[NotificationWizard] Обработчик кнопки 'toggle_weekly_report' вызван`
  );
  await ctx.answerCbQuery();
  ...
  if (!userId) {
    logger.error(
      "[NotificationWizard] User ID is undefined in toggle_weekly_report handler"
    );
    await ctx.reply(
      "Ошибка: не удалось определить пользователя. Начните сначала."
    );
  ...
  try {
    ...
    logger.error(
      "[NotificationWizard] Error in toggle_weekly_report handler:",
      error
    );
    await ctx.reply(
      "Произошла ошибка при обновлении настроек уведомлений. Попробуйте еще раз."
...
notificationWizardScene.action("change_views_threshold", async (ctx) => {
  logger.info(
    `[NotificationWizard] Обработчик кнопки 'change_views_threshold' вызван`
  );
  await ctx.answerCbQuery();
...
notificationWizardScene.action("change_notification_time", async (ctx) => {
  logger.info(
    `[NotificationWizard] Обработчик кнопки 'change_notification_time' вызван`
  );
  await ctx.answerCbQuery();
