...
function clearSessionState(
  ...
  if (ctx.scene.session) {
    logger.info(
      `[ScrapingWizard] Clearing session state before leaving (reason: ${reason})`
    );
...
async function safeSceneTransition(
  ...
  try {
    logger.info(
      `[ScrapingWizard] Transitioning to ${targetScene} scene (reason: ${reason})`
    );
    await ctx.scene.enter(targetScene);
  } catch (error) {
    logger.error(
      `[ScrapingWizard] Error entering ${targetScene} scene:`,
      error
    );
...
export const scrapingWizardScene = new Scenes.WizardScene<ScraperBotContext>(
  ...
  async (ctx) => {
    ...
    logger.debug(
      `[ScrapingWizard] Содержимое ctx.wizard.state:`,
      ctx.wizard.state
    );
    ...
    try {
      ...
      if (competitors.length === 0 && (!hashtags || hashtags.length === 0)) {
        message +=
          "⚠️ У вас нет добавленных конкурентов или хештегов для скрапинга.\n";
        message +=
          "Пожалуйста, добавьте конкурентов или хештеги перед запуском скрапинга.";
      ...
      if (hashtags && hashtags.length > 0) {
        keyboard.push([
          Markup.button.callback("📊 Скрапить хештеги", `scrape_hashtags`),
        ]);
      }
  ...
  async (ctx) => {
    ...
    keyboard.push([
      Markup.button.callback("🔄 Скрапить все хештеги", `scrape_all_hashtags`),
    ]);
    keyboard.push([Markup.button.callback("🔙 Назад", `back_to_menu`)]);
