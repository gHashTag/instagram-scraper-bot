...
// Импорт с any типом для модуля input
...
interface TestStepSend {
  action: "send";
  message: string;
  waitForResponse?: boolean;
}

interface TestStepCheckButton {
  action: "checkButton";
  button: string;
}

interface TestStepClickButton {
  action: "clickButton";
  button: string;
  waitForResponse?: boolean;
}

type TestStep = TestStepSend | TestStepCheckButton | TestStepClickButton;
...
if (!API_ID || !API_HASH) {
  console.error(
    "Ошибка: Необходимо указать TELEGRAM_API_ID и TELEGRAM_API_HASH в .env файле"
  );
  process.exit(1);
}
...
const testScenarios: TestScenario[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        action: "send",
        message: "https://www.instagram.com/example",
        waitForResponse: true,
      },
...
async function main() {
  ...
  console.log(
    "Сохраните эту строку сессии в .env файл как TELEGRAM_SESSION_STRING:"
  );
  console.log(client.session.save());

  // Получаем информацию о боте
  const botEntity = await client.getEntity(BOT_USERNAME);
  console.log(`Бот найден: ${(botEntity as any).username}`);
  ...
  client.addEventHandler(async (event: NewMessageEvent) => {
    ...
    if ((message.peerId as any).userId === (botEntity as any).id) {
  ...
  async function clickButton(
    ...
    if (!lastBotMessage || !lastBotMessage.replyMarkup) {
      console.log(
        `Не удалось нажать на кнопку "${buttonText}": нет разметки кнопок`
      );
      return null;
    }
    ...
    console.log(
      `Не удалось нажать на кнопку "${buttonText}": кнопка не найдена`
    );
    return null;
  }
  ...
  for (const scenario of testScenarios) {
    ...
    for (const step of scenario.steps) {
      switch (step.action) {
        case "send":
          await sendMessage(step.message, step.waitForResponse || false);
          break;
        case "checkButton":
          checkButton(step.button);
          break;
        case "clickButton":
          await clickButton(step.button, step.waitForResponse || false);
          break;
        default:
