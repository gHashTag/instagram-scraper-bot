...
function checkButtonHandlers() {
  console.log(
    "Проверка регистрации обработчиков кнопок в сцене конкурентов..."
  );
  ...
  console.log(
    `Найдено ${actionHandlers.length} обработчиков действий в сцене конкурентов`
  );
  ...
  actionHandlers.forEach((handler: any, index: number) => {
    ...
    console.log(
      `  Тип: ${typeof handler.predicate === "function" ? "Функция" : typeof handler.predicate}`
    );
...
function testButtonRegistration() {
  ...
  console.log(
    `Найдено ${actionHandlers.length} обработчиков действий в тестовой сцене`
  );
