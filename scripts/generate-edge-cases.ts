...
const generatePerformance =
  args.includes("--performance") || args.includes("--all");
    ...
    competitors: [
      {
        ...
        instagramUrl:
          "https://www.instagram.com/competitor_for_nonexistent_project",
      },
      {
        projectId: 1,
...
function generateEdgeCaseData(db: Database) {
  ...
  for (const project of config.edgeCases.projects) {
    try {
      ...
      log(
        `✅ Добавлен проект с граничным случаем: "${project.name.substring(0, 30)}${project.name.length > 30 ? "..." : ""}"`
      );
    } catch (error) {
  ...
  for (const competitor of config.edgeCases.competitors) {
    try {
      ...
      log(
        `✅ Добавлен конкурент с граничным случаем: "${competitor.username.substring(0, 30)}${competitor.username.length > 30 ? "..." : ""}"`
      );
    } catch (error) {
  ...
  for (const hashtag of config.edgeCases.hashtags) {
    try {
      ...
      log(
        `✅ Добавлен хештег с граничным случаем: "${hashtag.name.substring(0, 30)}${hashtag.name.length > 30 ? "..." : ""}"`
      );
    } catch (error) {
  ...
  for (const reel of config.edgeCases.reels) {
    try {
      ...
      log(
        `✅ Добавлен Reel с граничным случаем: "${reel.instagramId.substring(0, 30)}${reel.instagramId.length > 30 ? "..." : ""}"`
      );
    } catch (error) {
...
function generatePerformanceData(db: Database) {
  ...
  try {
    const result = db
      ...
      .get(1, "Performance Test Project") as { id: number };
    performanceProjectId = result.id;
    log(
      `✅ Создан проект для тестирования производительности (ID: ${performanceProjectId})`
    );
  } catch (error) {
    log(
      `❌ Ошибка при создании проекта для тестирования производительности: ${error}`
    );
    return;
  }
  ...
  const competitors = config.performanceData.generateCompetitors(
    competitorsCount,
  ...
  const hashtags = config.performanceData.generateHashtags(
    hashtagsCount,
    performanceProjectId
  );
  for (const hashtag of hashtags) {
    try {
      db.prepare("INSERT INTO Hashtags (project_id, name) VALUES (?, ?)").run(
        hashtag.projectId,
        hashtag.name
      );
    } catch (error) {
  ...
  const reels = config.performanceData.generateReels(
    reelsCount,
    performanceProjectId,
    "competitor",
    1
...
function main() {
  ...
  if (!fs.existsSync(config.dbPath)) {
    ...
    console.log(
      "Сначала запустите скрипт подготовки тестовой среды: bun run scripts/prepare-test-env.ts"
    );
    process.exit(1);
  }
  ...
  console.log(
    "3. Следуйте инструкциям в чек-листе: docs/MVP_TESTING_CHECKLIST.md"
  );
}
