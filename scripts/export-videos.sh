...
# Скрипт для экспорта видео Reels через Node.js
# Использование: ./scripts/export-videos.sh <projectId> [sourceType] [sourceId] [limit] [outputDir]
# Где <sourceType> может быть: competitor, hashtag или all

# Проверяем количество аргументов
if [ "$#" -lt 1 ]; then
  echo "Использование: ./scripts/export-videos.sh <projectId> [sourceType] [sourceId] [limit] [outputDir]"
...
OUTPUT_DIR=${5:-"./exports/videos"}  # По умолчанию "./exports/videos"
...
# Запускаем экспорт видео
echo "Запуск экспорта видео для проекта $PROJECT_ID, тип источника: $SOURCE_TYPE, ID источника: $SOURCE_ID, лимит: $LIMIT, директория: $OUTPUT_DIR"
npx tsx src/scripts/export-videos.ts $PROJECT_ID $SOURCE_TYPE $SOURCE_ID $LIMIT "$OUTPUT_DIR"
